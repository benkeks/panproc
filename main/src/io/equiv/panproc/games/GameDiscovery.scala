...
/** this function can add a `predecessors`-function to a game that is only specified in terms of
  * some given nodes and the `successors`-function. it also adds a map for the number of successors
