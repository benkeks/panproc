...
case class Coloring[E](val colors: Map[E, Coloring.Color] = Map()):
  ...
  def filter(f: E => Boolean) =
    Coloring[E](colors.view.filterKeys(f).toMap)
...
//    Coloring(colors ++ updates.flatten)
  ...
  /** normalizes the color space to 0..n-1 for n colors, where the color order is the order of the
  ...
  /** normalizes the color space to 0..n-1 for n colors, where the color order is the old color
    * order
    */
