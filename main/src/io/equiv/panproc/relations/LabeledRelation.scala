...
class LabeledRelation[E, L](val rep: Map[E, Map[L, Set[E]]]):
  ...
  def peekPath(e: E): List[E] =
    rep.get(e).flatMap(_.headOption.flatMap { case (l, ee2) =>
      ee2.headOption.map(e :: peekPath(_))
    }).getOrElse(List())
  ...
  def symmetricReflexiveClosure(reflexiveOn: Iterable[E], reflexiveEdge: L) =
    new LabeledRelation(tupleSet ++ tupleSet.map { case (e1, l, e2) =>
      (e2, l, e1)
    } ++ reflexiveOn.map(e => (e, reflexiveEdge, e)))
  ...
  def getReachablePart(startingSet: Iterable[E]) =
    ...
    while queue.nonEmpty do
      ...
      if !visited(e) then
        ...
        for lee2 <- rep.get(e); ee2 <- lee2.values do
          queue ++= ee2
  ...
  /** If relation (restricted to selected labels) is an equivalence relation, outputs an equivalent
    * "coloring" of E
    */
  def toQuotientColoring(labelFilter: L => Boolean, representative: (E, E) => E): Coloring[E] =
    val colors = lhs.groupBy { e1 =>
      ...
      post.fold(e1)(
        representative
      ) // if the relation is irreflexive, a loop will be assumed anyways
    }
  ...
  def toGraphString() =
    ...
    tupleSet.map { case (e1, l, e2) =>
      ...
      s"${idFor(e1)}->${idFor(e2)}" + (if label != "" then "[label=\"" + label + "\"]" else "")
  ...
  override def equals(that: Any) = that match
    case t: LabeledRelation[?, ?] => rep == t.rep
    case _                        => false
