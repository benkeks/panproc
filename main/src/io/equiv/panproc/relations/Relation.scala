...
class Relation[E](val rep: Map[E, Set[E]]):
  ...
  lazy val inverseRep: Map[E, Set[E]] = tupleSet.map(_.swap).groupBy(_._1).view.mapValues(_.map(_._2)).toMap

  lazy val lhs = rep.keySet
  lazy val rhs = rep.values.flatten.toSet
  lazy val domain = lhs ++ rhs

  def merge(other: Relation[E]) = new Relation(this.tupleSet ++ other.tupleSet)

  def +(e1: E, e2: E) = new Relation(this.tupleSet + (e1 -> e2))
  ...
  def transitiveClosureFast =
    ...
    val connections = HashSet(tupleSet.toSeq*)
  ...
  def inverse = new Relation[E](inverseRep)
  ...
  def getPathsFrom(start: Set[E]): Set[List[E]] =
    val comp = FixedPointCollection[List[E]] { paths =>
      paths.flatMap { p =>
        values(p.last).map(next => p :+ next) // all one step extensions from its end
      }
    }
  ...
  def toGraphString(edgeLabeling: (E, E) => String) =
    ...
    tupleSet.map {
      case (l, r) =>
        ...
        s"${idFor(l)}->${idFor(r)}" + (if label != "" then "[label=\"" + label + "\"]" else "")
    }.mkString("digraph rel{\n  ", ";\n  ", "}")

  override def toString = tupleSet.mkString("{", ",", "}")

  override def equals(that: Any) = that match
    case t: Relation[?] => rep == t.rep
    case _              => false
