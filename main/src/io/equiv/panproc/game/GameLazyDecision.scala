...
trait GameLazyDecision[P] extends GameDiscovery:
  ...
  private val computedPredecessors =
    collection.mutable.Map[GameNode, Set[GameNode]]() withDefaultValue Set()

  override def successors(gn: GameNode): Iterable[GameNode] = computedSuccessors(gn)
  private val computedSuccessors =
    collection.mutable.Map[GameNode, Set[GameNode]]() withDefaultValue Set()
  ...
  def isAttackerWinningPrice(gn: GameNode, p: P) =
    attackerVictoryPrices(gn).exists(mwp => priceIsBetterOrEq(mwp, p))
  ...
  private def priceUpdate(node: GameNode, newPrices: Iterable[P]) =
    ...
    val interestingNewPrices =
      newPrices.filterNot(p => oldPrices.exists(op => priceIsBetterOrEq(op, p)))
    if interestingNewPrices.nonEmpty then
      val interestingOldPrices =
        oldPrices.filterNot(p => interestingNewPrices.exists(priceIsBetter(_, p)))
      attackerVictoryPrices(node) = interestingOldPrices ++ interestingNewPrices.filterNot(p =>
        interestingNewPrices.exists(op => priceIsBetter(op, p))
      )
      true
    else
      false
  ...
  def populateGame(
    ...
    while todo.nonEmpty do
      if printToDoLength then println(todo.size)
      val currNode = todo.dequeue()
      val instaWin = instantAttackerWin(currNode)
      if instaWin.nonEmpty then
        if priceUpdate(currNode, instaWin) then
          // propagate a new win
          predecessors(currNode).foreach(_ +=: todo)
      else
        if !(visited contains currNode) then
          visited += currNode
          ...
          do
            ...
            if !(discovered contains gn) then
              discovered += gn
              todo += gn
        val updatedPrice = computeCurrentPrice(currNode)
        if priceUpdate(currNode, updatedPrice) then
          // propagate a new win
          predecessors(currNode).foreach(_ +=: todo)
